...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveAnswer() {
      ...
      return fetch("{{ route('student.exams.answer', [$exam->exam_code, $currentQuestion->kode_soal]) }}", {
          method: 'POST',
          headers: {
    ...
    function updateNavigationButtons() {
      const currentButton = document.querySelector(`.question-nav[data-kode-soal="{{ $currentQuestion->kode_soal }}"]`);
      if (currentButton) {
        const selectedAnswer = document.querySelector('input[name="answer"]:checked');
        const markDoubtCheckbox = document.getElementById('markDoubtCheckbox');
      ...
      button.addEventListener('click', function(e) {
        ...
        saveAnswer().then(success => {
          if (success) {
            ...
            window.location.href = "{{ route('student.exams.do', [$exam->exam_code, '']) }}/" + kodeSoal;
          } else {
    ...
    if (document.getElementById('prevQuestion')) {
      document.getElementById('prevQuestion').addEventListener('click', () => {
        saveAnswer().then(success => {
          if (success) {
            window.location.href = "{{ route('student.exams.do', [$exam->exam_code]) }}/{{ $prevQuestion ? $prevQuestion->kode_soal : '' }}";
          }
        });
      });
    }
    ...
    if (document.getElementById('nextQuestion')) {
      document.getElementById('nextQuestion').addEventListener('click', () => {
        saveAnswer().then(success => {
          if (success) {
            window.location.href = "{{ route('student.exams.do', [$exam->exam_code]) }}/{{ $nextQuestion ? $nextQuestion->kode_soal : '' }}";
          }
        });
      });
    }
        ...
        if (confirmed) {
          ...
          saveAnswer().then(success => {
            if (success) {
              ...
              showAlert('Gagal menyimpan jawaban terakhir. Silakan coba lagi.');
            }
          });
        }
      });
    ...
    window.addEventListener('beforeunload', function(e) {
      // Hanya trigger untuk event sebelum unload, tidak untuk navigasi internal
