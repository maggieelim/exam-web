...
@section('content')
    ...
    <div class="collapse card" id="filterCollapse">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="answer_status" id="answer_status" class="form-select">
                        <option value="all"
                            {{ request('answer_status') == 'all' || !request('answer_status') ? 'selected' : '' }}>All
                            Answers</option>
                        <option value="correct" {{ request('answer_status') == 'correct' ? 'selected' : '' }}>Correct
                        </option>
                        <option value="incorrect" {{ request('answer_status') == 'incorrect' ? 'selected' : '' }}>Incorrect
                        </option>
                        <option value="not_answered" {{ request('answer_status') == 'not_answered' ? 'selected' : '' }}>Not
                            Answered</option>
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="feedback_status" id="feedback_status" class="form-select">
                        <option value="all"
                            {{ request('feedback_status') == 'all' || !request('feedback_status') ? 'selected' : '' }}>All
                        </option>
                        <option value="with_feedback"
                            {{ request('feedback_status') == 'with_feedback' ? 'selected' : '' }}>With Feedback</option>
                        <option value="without_feedback"
                            {{ request('feedback_status') == 'without_feedback' ? 'selected' : '' }}>Without Feedback
                        </option>
                    </select>
                </div>
    ...
    @if (!empty($question['student_feedback'])) border-start border-4 border-info @endif">
            <div class="card-body">
                ...
                <div class="d-flex flex-wrap justify-content-end align-items-center gap-2 mb-2">
                    ...
                    <small class="badge bg-primary ">{{ $question['category'] }}</small>
                </div>
                ...
                <!-- Feedback -->
                <div class="mt-3">
                    <p class="fw-bold d-block mb-1">Feedback for this question:</p>
