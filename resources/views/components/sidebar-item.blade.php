...
@php
$context = session('context'); // pssk / pspd
$fullPattern = $context ? $context . '/' . $pattern : $pattern;

$active = request()->is($fullPattern);$url = isset($route) ? route($route, $params) : (isset($url) ? url($url) : '#');
@endphp
