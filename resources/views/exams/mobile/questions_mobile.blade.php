...
@section('content')
    <div>
        <div>
            <h5 class="mb-2">Manage Questions - {{ $exam->title }}</h5>
        </div>
        <div class="d-flex gap-2 justify-content-end">
            @if ($exam->status === 'upcoming')
                <button class="btn btn-sm btn-primary d-flex align-items-center justify-content-center" data-bs-toggle="modal"
                    data-bs-target="#reuploadModal" style="height: 32px;">
                    Reupload Questions
                </button>
            @endif
                        ...
                        <div class="row">
                            <div class="d-flex flex-column h-100">
                                ...
                                <div class="mb-0">
                                    ...
                                    <input type="hidden" name="delete_image" id="delete_image-{{ $question->id }}"
                                        value="0">
                                ...
                                <div class="d-flex gap-2">
                                    <button type="submit" class="btn flex-fill btn-sm btn-primary update-btn">Update
                                        Soal</button>
                                    <button type="button" class="btn flex-fill btn-sm btn-warning anulir-btn"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.question-form').forEach(form => {
                form.addEventListener('submit', function(e) {
                    ...
                    fetch(`{{ env('APP_URL') }}/exams/{{ $exam->exam_code }}/questions/${questionId}`, {
                            method: 'POST',
                            body: formData,
                            headers: {
            ...
            // Handle delete soal
            // Handle delete soal - VERSI YANG DIPERBAIKI
            document.querySelectorAll('.delete-question-btn').forEach(button => {
                button.addEventListener('click', function() {
                    const questionId = this.dataset.questionId;
                    const examCode = '{{ $exam->exam_code }}'; // Ambil exam code dari Blade
                    ...
                    fetch(`/exams/${examCode}/questions/${questionId}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'X-Requested-With': 'XMLHttpRequest',
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                showNotification(data.message, 'success');
                                // Hapus card soal dari DOM dengan animasi
                                ...
                                if (questionElement) {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        // Refresh halaman jika tidak ada soal lagi
                                        if (document.querySelectorAll('.question-card')
                                            .length === 0) {
