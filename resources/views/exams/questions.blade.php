  ...
  <div class="modal fade" id="reuploadModal" tabindex="-1" aria-labelledby="reuploadModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <form action="{{ route('exams.questions.updateByExcel', $exam->exam_code) }}" method="POST" enctype="multipart/form-data">
          @csrf
      ...
      <div class="mx-3 my-2 py-2">
        <div class="row g-2">
          <div class="col-md-8">
            ...
            <input type="text" name="search" class="form-control" placeholder="Cari Soal Ujian" value="{{ request('search') }}">
          </div>
          <div class="col-md-4">
  ...
  <div class="card mb-4 question-card" id="question-{{ $question->id }}">
    <div class="card-header pb-0 px-3 d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex gap-3">
        ...
        <button type="button" class="btn btn-lg btn-link p-0 delete-question-btn" title="Hapus Soal" data-question-id="{{ $question->id }}">
    ...
    <div class="card-body pt-0 p-3">
      <form class="question-form" data-question-id="{{ $question->id }}">
        ...
        <div class="mb-3">
          ...
          <input type="file" name="image" id="image-{{ $question->id }}" class="form-control" accept="image/*">
          ...
          <div id="image-container-{{ $question->id }}" class="mt-2">
            ...
            <div class="position-relative d-inline-block existing-image">
              ...
              <button type="button" class="btn btn-danger btn-sm rounded-circle align-items-center justify-content-center position-absolute delete-image-btn" style="width: 30px; height: 30px; padding: 0; top: -10px; right: -10px;" data-question-id="{{ $question->id }}">
                x
              </button>
            </div>
            @endif
            ...
            <div class="col-md-6 mb-2">
              <div class="d-flex align-items-center">
                <div class="form-check me-2">
                  <input type="checkbox" name="options[{{ $option->id }}][is_correct]" class="form-check-input" {{ $option->is_correct ? 'checked' : '' }}>
                </div>
        ...
        <div class="d-flex gap-2">
          ...
          <button type="button" class="btn btn-sm btn-warning anulir-btn" data-question-id="{{ $question->id }}">Anulir Soal</button>
        </div>
      </form>
    </div>
  </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
          ...
          .then(data => {
            if (data.success) {
              ...
              if (data.data && data.data.image_url !== undefined) {
                updateImageDisplay(questionId, data.data.image_url, data.data.has_image);
              }
            } else {
    ...
    document.querySelectorAll('.anulir-btn').forEach(button => {
      button.addEventListener('click', function() {
        ...
        if (!confirm('Yakin ingin menganulir soal ini? Semua jawaban akan dianggap benar.')) {
          return;
        }
