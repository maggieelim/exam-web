...
class ExamController extends Controller
{
    ...
    public function index(Request $request, $status = null)
    {
        ...
        $viewMobile = $user->hasRole('student')
            ? 'students.exams.index'
            : 'pssk.exams.mobile.index_mobile';
            ...
            if ($status === 'previous') {
                $query->where(function ($q) use ($user) {
                    $q->where('status', 'ended')
                        ->orWhereHas('attempts', function ($sub) use ($user) {
                ...
                $query->whereIn('status', ['upcoming', 'ongoing'])->whereDoesntHave('attempts', fn($sub) => $sub->where('user_id', $user->id)->whereIn('status', ['completed', 'timeout']));
            }
        }
    }
    ...
    private function mapExamAttributes($exam)
    {
        ...
        $exam->has_completed = $userAttempt && ($userAttempt->status === 'completed' || $userAttempt->status === 'timeout');
        $exam->has_ongoing = $userAttempt && $userAttempt->status === 'in_progress';
    ...
    public function create()
    {
        ...
        return view('pssk.exams.create', compact('courses', 'semesters', 'activeSemester'));
    }
    ...
    public function show(Request $request, string $exam_code)
    {
        ...
        return view('pssk.exams.show', compact('exam', 'questions', 'status', 'total_participants'));
    }
    ...
    public function edit($status, string $exam_code)
    {
        ...
        return view('pssk.exams.edit', compact('status', 'exam', 'courses'));
    }
