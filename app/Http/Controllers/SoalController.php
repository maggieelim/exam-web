...
{
    $kodes = \App\Models\Soal::select('kode_soal', 'nama_blok')
        ->distinct()
        ->get();
...
{
    $soals = \App\Models\Soal::where('kode_soal', $kode)->get();
    return view('soal.show_by_kode', compact('soals', 'kode'));
}

private function generateKodeSoal()
{
    $lastSoal = \App\Models\Soal::orderBy('id', 'desc')->first();
